...
public:
	...
	void setShaderParameters(ID3D11Device* device, ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture0, ID3D11ShaderResourceView* texture1, ID3D11ShaderResourceView * heightMap, ID3D11ShaderResourceView* shadowMap, LightManager* lm, float* edges, float* inside, float texRes, float height);
