...
static inline float  ImLinearSweep(float current, float target, float gravity)    { if (current < target) return ImMin(current + gravity, target); if (current > target) return ImMax(current - gravity, target); return current; }
