 ...
 * Base shader is the parent for other custom shader objects. Offers required functions and a
...
class BaseShader
{
  ...
  public:
	...
	virtual void renderDepth(ID3D11DeviceContext* deviceContext, int vertexCount);
  ...
  protected:
	...
	void loadColourVertexShader(
		const wchar_t* filename); ///< Load Vertex shader, pre-made for position and colour only
	void loadTextureVertexShader(
		const wchar_t* filename); ///< Load Vertex shader, pre-made for position and tex only
	...
	void loadDepthPixelShader(const wchar_t* filename); ///< Load Depth Pixel shader
